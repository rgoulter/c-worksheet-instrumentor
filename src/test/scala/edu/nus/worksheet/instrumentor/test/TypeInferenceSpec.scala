...
class TypeInferenceSpec extends AnyFlatSpec {
  ...
  it should "work for primary expressions" in {
    assertInference(new PrimitiveType("\"Abc\"", "char *"), null, "\"Abc\"");
    assertInference(
      new PrimitiveType("\"Abcdef\"", "char *"),
      null,
      "\"Abc\" \"def\""
    );
  ...
  it should "infer postfix expressions" in {
    assertInference(
      new PrimitiveType("x[0]", "int"),
      "int x[2] = {1,2};",
      "x[0]"
    );
    assertInference(
      new PrimitiveType("s.x", "int"),
      "struct {int x;} s;",
      "s.x"
    );
    assertInference(
      new PrimitiveType("(*p).x", "int"),
  ...
  it should "infer pointers, referenced like arrays" in {
  ...
  it should "infer postfix function calls" in {
    assertInference(
      new PrimitiveType("f()", "int"),
      "int f(int x) { return 3; }",
      "f()"
    );
    assertInference(
      new PrimitiveType("(*g)(3)", "int"),
      "int (*g)(int);",
      "(*g)(3)"
    );
    ...
    assert(
      voidOfFunProto == voidOfFunDefn,
      "void func definition & prototype should return the same."
    );

    assertResult(
      null,
      "inferred type from calling void (proto). Got " + voidOfFunProto
    )(voidOfFunProto);
    assertResult(
      null,
      "inferred type from calling void (defn). Got " + voidOfFunDefn
    )(voidOfFunDefn);
  }
  ...
  it should "infer postfix compound literals" in {
    assertInference(
      new StructType(
        ...
        "unIntFloat",
        Seq(
          new PrimitiveType("(union unIntFloat) { i }.i", "int"),
  ...
  it should "infer postfix compound literals (arrays)" in {
    ...
    // (int[256]){ [' '] = 1, ['\t'] = 1, ['\n'] = 1, ['\r'] = 1 }
    // should be ptr
    assertInference(expected, null, cmpdLitArr);

    // *(int[256]){ [' '] = 1, ['\t'] = 1, ['\n'] = 1, ['\r'] = 1 }
    // should be int. Amazingly.
    assertInference(derefArr, null, "*" + cmpdLitArr);
  }

  it should "infer postfix compound literals (arrays of pointers)" in {
    val cmpdLitPtrs = """(int *[]) { p, q }""";

    val expectedPtr = new PointerType(
      cmpdLitPtrs,
      new PointerType(
        s"(*$cmpdLitPtrs)",
        new PrimitiveType(s"(*(*$cmpdLitPtrs))", "int")
      )
    );
...
int* f1() {
  ...
  it should "infer postfix compound literals (anonymous structs)" in {
    ...
    assertInference(
      new PrimitiveType("(struct { int x; float y; }) { 3, 4.5f }.x", "int"),
      null,
      "(struct { int x; float y; }) { 3, 4.5f }.x"
    );
  }
  ...
  it should "infer infix expressions" in {
    ...
    assertInference(new PrimitiveType("!i", "int"), "int i;", "!i");
    assertInference(
      new PointerType("&i", new PrimitiveType("i", "int")),
      "int i;",
      "&i"
    );
  ...
  it should "infer pointer-arithmetic expressions" in {
    ...
    assertInference(
      new PointerType("(p + 3)", new PrimitiveType("(*(p + 3))", "int")),
      "int *p;",
      "p + 3"
    );
  }
